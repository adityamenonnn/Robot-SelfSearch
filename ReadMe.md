...
gcc -o C_Coursework main.c grid.c robot.c marker.c obstacle.c layers.c graphics.c -lm
./C_Coursework | java -jar drawapp-4.5.jar
...
int isInsideOctagon(int row, int col, int rows, int cols) // LLM Generated Code
{
    ...
    int maxDist;
    if (rows < cols) {maxDist = rows / 2 - 2;} 
    else {maxDist = cols / 2 - 2;}
